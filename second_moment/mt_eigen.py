...
def eval_Mt_vec_prod(vec, model, dataloader, device):
    model.eval()  # in evaluation mode, the parameters won't change
    #    model.zero_grad() # clears grad for every parameter in the net

    num_params = sum(p.numel() for p in filter(lambda p: p.requires_grad, model.parameters()))
    ...
    for i, (data, labels) in enumerate(dataloader):
        ...
        grads = torch.autograd.grad(loss, filter(lambda p: p.requires_grad, model.parameters()))  # tuple
        gt = to_vector(grads)
...
def lanczos_tridiag(model, train_loader, device, M, sig2=0):
    """
    Inputs:
...
def lanczos_tridiag_with_rop_residual(args, model, dataloader, device, M):
    ...
    for m in range(M):
        ...
        v = eval_Mt_vec_prod_with_rop(args, q, model, dataloader, device) - eval_hess_vec_prod_with_rop(args, q, model,
                                                                                                        dataloader,
                                                                                                        device)
